...
const getGatewayRedirectUrl = (payment: any) => {
...
export const createPayment = async (req: Request, res: Response) => {
  try {
    ...
    if (!resolvedBookingId) {
      ...
      const unavailable = seats.filter((id: string) => !show.isSeatAvailable(id));
      if (unavailable.length > 0) {
        return res.status(400).json({ success: false, message: `Seats not available: ${unavailable.join(", ")}` });
      }

      const booking = new Booking({ userId: (req.user as any)?._id, showId, seatIds: seats, status: "Pending" });
...
export const paymentSuccess = async (req: Request, res: Response) => {
  try {
    const { transactionUUID, refId } = (req.body || req.query) as any;
    ...
    if (booking) {
      await (booking as any).confirmBooking();
    }
...
export const paymentFailed = async (req: Request, res: Response) => {
  try {
    const { transactionUUID, reason } = (req.body || req.query) as any;
    if (!transactionUUID) return res.status(400).json({ success: false, message: "Invalid request" });
...
export const getMyTickets = async (req: Request, res: Response) => {
  try {
    ...
    const showIds = payments.map((p: any) => p.showId).filter(Boolean);
    const shows = await Show.find({ _id: { $in: showIds } }).select("startTime status");
    const showMap = new Map(shows.map((s: any) => [s._id.toString(), s]));

    const tickets = payments.map((p: any) => {
...
export const cancelTicket = async (req: Request, res: Response) => {
  try {
    ...
    const minutesSinceBooking = (Date.now() - new Date((payment as any).createdAt).getTime()) / 60000;
    ...
    await (booking as any).cancelBooking();
    await (payment as any).markAsRefunded();
